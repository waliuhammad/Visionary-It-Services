              ...
              <div key={order.id} className="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden transition-all">
                ...
                >
                  <div className="flex items-center gap-6">
                    <div>
                      <p className="text-xs font-mono text-neutral-400 mb-1">#{order.id.slice(-6).toUpperCase()}</p>
                      <p className="font-semibold text-neutral-900">{order.customerInfo?.name || 'Guest'}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
                      ...
                      <div>
                        ...
                        <div className="bg-white p-4 rounded-xl border border-neutral-100 text-sm space-y-2">
                          <p><span className="text-neutral-500 w-20 inline-block">Email:</span> {order.customerInfo?.email}</p>
                          <p><span className="text-neutral-500 w-20 inline-block">Phone:</span> 
                            {order.customerInfo?.phone}
                            {order.customerInfo?.phone && (
                              <a href={`https://wa.me/${order.customerInfo.phone.replace(/\D/g,'')}`} target="_blank" rel="noreferrer" className="ml-2 text-emerald-500 hover:underline text-xs">
                                WhatsApp ↗
                              </a>
                            )}
                          </p>
                          <p><span className="text-neutral-500 w-20 inline-block align-top">Address:</span> 
                            <span className="inline-block w-[calc(100%-5rem)]">{order.customerInfo?.address}</span>
                          </p>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-neutral-100 divide-y divide-neutral-100">
                          {order.items?.map((item, idx) => (
                            <div key={idx} className="p-3 flex items-center justify-between text-sm">
                              ...
                              <span className="text-neutral-500">Rs. {(item.price * item.quantity).toLocaleString()}</span>
                            </div>
                          ))}
                          <div className="p-3 flex justify-between bg-neutral-50 text-sm">
                            <span className="text-neutral-500">Subtotal</span>
                            <span>Rs. {(order.total - (order.shippingFee || 0)).toLocaleString()}</span>
                          </div>
                          <div className="p-3 flex justify-between bg-neutral-50 text-sm">
                            <span className="text-neutral-500">Shipping</span>
                            <span>Rs. {order.shippingFee?.toLocaleString() || 0}</span>
                          </div>
