...
export default function Categories() {
  ...
  return (
    <div>
      <PageHeader
        firstWord="Store" secondWord="Categories" accentColor="text-brand-500"
        subtitle="Organize your product catalog"
      >
        <PillButton label="New Category" icon={FolderPlus} onClick={handleCreate} className="bg-brand-500 hover:bg-brand-600 shadow-brand-500/20" />
      </PageHeader>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((c, i) => (
            <div key={c.id || i} className="bg-white rounded-3xl p-6 shadow-sm flex flex-col justify-between group hover:shadow-md transition-shadow relative overflow-hidden">
              <div className="absolute -right-4 -top-4 w-24 h-24 bg-brand-50 rounded-full opacity-50 group-hover:scale-150 transition-transform duration-500" />
              <div className="relative z-10">
                ...
                <div className="mt-auto pt-4 border-t border-neutral-100 flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-brand-600 bg-brand-50 px-2.5 py-1 rounded-full">
